...
| 項目 | 選定 | 理由 |
|------|------|------|
| Web フレームワーク | **FastAPI**（2026-07-05 決定） | multipart・バリデーション（Pydantic）・OpenAPI ドキュメントが標準。presentation 層はどうせ書き直すため移行差分は小さい |
...
- 運用コスト: 現行の ghcr.io 配布 + ローカル実行（0 円）を維持。**クラウドへのデプロイは本 PDR のスコープ外**（参考: 必要になった場合は Cloud Run のゼロスケール + 無料枠で 0 円運用が成立する見込み）
...
| 7.（任意）性能追加 | 隣接ペア限定マッチング（`cv2.detail`） | 多数枚ベンチマークで効果確認 |
...
1. **Web フレームワーク**: FastAPI を採用 — 2026-07-05 決定
2. **クラウドデプロイ**: スコープ外。現行の ghcr.io 配布 + ローカル実行を維持 — 2026-07-05 決定
   (2026-07-05 上書き: [PDR-zero-cost-hosting.md](./PDR-zero-cost-hosting.md) で 0 円ホスティングとして再検討)
