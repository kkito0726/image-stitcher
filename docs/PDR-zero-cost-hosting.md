...
最悪ケースでも「無料枠を使い切って 503 になる」方向に倒れ、高額請求には至らない。アプリ全体への認証(トークン・Turnstile 等)は本 PDR ではスコープ外とし、悪用が実際に観測されたら追加する。

**2026-07-05 更新**: フル解像度ダウンロード(コストの主要因、§4 参照)に限っては、[PDR-preview-lazy-download.md](./PDR-preview-lazy-download.md) §4.3 で共有パスワードによる保護を追加した(本番のみ有効)。これにより野良ユーザーによるフル解像度の無制限ダウンロードというコストリスクの主要因には既に対処済みで、本項の「悪用対策は将来検討」はプレビュー閲覧・リクエスト全般への追加対策(レート制限等)の話として引き続き保留する。

**2026-09-23 更新**: 自宅サーバー + Cloudflare Tunnel での運用に切り替えたため、上記パスワード保護は撤回・削除した。
